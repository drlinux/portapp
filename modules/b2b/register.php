...
switch($_action)
{
	...
	case 'saveUser':
		if ($model->isValidRegisterForm($_POST)) {
			echo(json_encode(array("success"=>$model->saveRegisterForm($_POST, $smarty->getVariable("_USER_ROLE_B2B"), $smarty->getVariable("_USER_INITIALSTATUS_B2B")), "msg"=>$model->msg, "field"=>$model->field)));
		}
		else {
