...
switch($_action)
{
	...
	case 'show':
		...
		if ($productId == null) header("Location: " . PROJECT_URL . "modules/privateshop/");
		$data = $model->getProductattributeByProductId($productId);
		//print_r($data);exit;
		
		$model->displayTemplate("privateshop", "product", $data);
		break;
	case 'view':
	default:
		$model->displayTemplate("privateshop", "product");
		break;
}