...
$(document).ready(function() {
	$('#example').dataTable( {
			...
			"aButtons": [
							{
								...
								"fnClick": function ( nButton, oConfig, oFlash ) {
									if (confirm('{#ALERT_AreYouSureToDeleteAll#}')) {
										location.href = '{$SCRIPT_NAME}?action=deleteVouchers';
									}
								},
		...
		"fnRowCallback": function( nRow, aData, iDisplayIndex ) {
			$('td:eq(0)', nRow).html( '<a href="{$SCRIPT_NAME}?action=edit&voucherId=' + aData[0] + '">' + aData[1] + '</a>' );
			//if (aData[3] != null) $('td:eq(2)', nRow).html( '<a href="{$SCRIPT_NAME}?action=edit&voucherId=' + aData[0] + '"><img src="img/voucher/100/' + aData[3] + '" /></a>' );
			return nRow;
		},
		"oLanguage": {
			"oPaginate": {
