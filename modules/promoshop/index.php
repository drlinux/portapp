...
switch($_action)
{
	case 'changeLanguage':
		...
		if(isset($_GET["uri"])) {
			...
			//$URI = $g_project->uri;
			$URI = PROJECT_URL . "modules/promoshop/";
		}
	...
	case 'breadcrumbsMenu':
		...
		echo($permission->jsonTree(53)); // TODO: Bu numara (53) permission tablosundaki PROMOSHOP ana menüye ait.
		break;
	
	
	case 'checkAuthenticated':
		echo(json_encode(array("authenticated"=>Permission::checkPermission("promoshop"))));
		break;
	case 'login':
		if (isset($_SESSION["userId"])) header("Location: " . PROJECT_URL . "modules/promoshop/");
		$model->displayTemplate("promoshop", "login_form", $_REQUEST);
		break;
	case 'logout':
		...
		header("Location: " . PROJECT_URL . "modules/promoshop/");
		break;
	...
	case 'submit':
		...
		if($model->isValidForm($_POST) && $model->authenticate($_POST)) {
			if (!isset($_POST["uri"]) || $_POST["uri"]=="") {
				$redir = PROJECT_URL . "modules/promoshop/";
			}
			else {
				$redir = $_POST["uri"];
			}
			...
			$data["uri"] = $_POST["uri"];
			$model->displayTemplate("promoshop", "login_form", $data);
		}
		break;
	...
	default:
		Permission::checkPermissionRedirect("promoshop");
		$model->displayTemplate("promoshop", "index");
		break;
}