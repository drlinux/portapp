 ...
 */
function smarty_outputfilter_javascriptpacker($source, $smarty)
{
	preg_match_all('/(<script.*>)(.*)(<\/script>)/imxsU', $source, $match);
	$_script_blocks = $match;
	//print_r($_script_blocks);exit;
    
	$source = preg_replace("!<script[^>]*?>.*?</script>!is", '@@@SMARTY:PACK:SCRIPT@@@', $source);
	//echo $source;exit;
    ...
    smarty_outputfilter_javascriptpacker_replace("@@@SMARTY:PACK:SCRIPT@@@", $_script_blocks, $source);

    return $source;
}

function smarty_outputfilter_javascriptpacker_replace($search_str, $replace, &$subject)
{
	...
	$_pos = 0;
	for ($_i=0, $_count=count($replace[2]); $_i<$_count; $_i++) {
		if (($_pos=strpos($subject, $search_str, $_pos))!==false) {
			$packer = new JavaScriptPacker($replace[2][$_i], $encoding, $fast_decode, $special_char);
			$packed = $packer->pack();
			$subject = substr_replace($subject, $replace[1][$_i].$packed.$replace[3][$_i], $_pos, $_len);
		}
		else {
			break;
		}
